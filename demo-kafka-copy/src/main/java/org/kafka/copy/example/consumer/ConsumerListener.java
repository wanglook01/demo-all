package org.kafka.copy.example.consumer;

import lombok.extern.slf4j.Slf4j;
import org.apache.kafka.clients.consumer.ConsumerRecord;
import org.kafka.copy.example.constant.TopicConst;
import org.kafka.copy.example.dto.SkuPriceChangeDTO;
...
public class ConsumerListener {
    ...
    public void skuPriceChangeConsume(List<ConsumerRecord<String, String>> recordList,
                                      Acknowledgment ack) {
        List<String> priceChangeList = recordList.stream().map(x -> JsonUtil.readValue(x.value(), SkuPriceChangeDTO.class)).map(x -> x.getFromPrice() + "->" + x.getToPrice()).toList();
